...
// Enhanced Task interface with new features
...
interface EnhancedTaskAction extends TaskAction {
  type: TaskAction['type'] | 'ADD_NOTE' | 'REMOVE_NOTE' | 'ADD_LINK' | 'REMOVE_LINK' | 'SET_ESTIMATED_DURATION' | 'UPDATE_ACTUAL_DURATION' | 'MIGRATE_DATA';
}
...
const enhancedTaskReducer = (state: TaskState, action: EnhancedTaskAction): TaskState => {
  switch (action.type) {
    case 'ADD_NOTE':
      return {
        ...state,
    ...
    case 'MIGRATE_DATA':
      ...
      return {
        ...
        tasks: state.tasks.map((task) => ({
          ...
          notes: task.notes || [],
          links: task.links || [],
          estimatedDuration: task.estimatedDuration || 25, // default 25 minutes
          actualDuration: task.actualDuration || 0
        })),
      };
    default:
      // Delegate to original reducer for existing actions
...
export const EnhancedTaskProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [state, dispatch] = useReducer(enhancedTaskReducer, initialState);
